...
def test_download():
    ...
    train_loader = ProteinLoader("./data/bestprot_test/valid", batch_size=8, node_features_type="chemical+sidechain_orientation+dihedral", rewrite=True)
    batch = next(iter(train_loader))
    assert set(batch.keys()) == {"X", "S", "mask", "mask_original", "residue_idx", "chain_encoding_all", "chain_id", "sidechain_orientation", "dihedral", "chemical"}
