...
async def _go(download_list, tmp_folder, snapshots):
    session = get_session()
    async with session.create_client("s3", config=Config(signature_version=UNSIGNED)) as client:
        tasks = [
            _download_file(
                client=client, 
                snapshots=snapshots,
...
def _singleProcess(download_list, tmp_folder, snapshots):
    ...
    return loop.run_until_complete(_go(download_list, tmp_folder=tmp_folder, snapshots=snapshots))
...
def _download_s3_parallel(pdb_ids, tmp_folder, snapshots):
    ...
    out = p_map(lambda x: _singleProcess(download_list=x, tmp_folder=tmp_folder, snapshots=snapshots), download_list_chunk)
    return out