...
def get_duplicate_weird_chains(data_scn, pdb_ids=None, dataset="train"):
    ...
    for key in tqdm(pdb_ids):
        chains = key_dict[key]
        main_chains = [
            x.split("_")[-1].lower() for x in chains if len(x.split("_")[-1]) <= 2
        ]
    ...
    for key in tqdm(pdb_ids):
        ...
        for weird_chain_id in weird_chain_ids:
            ...
            chain_list = sorted(
                [
                    x
                    for x in weird_chains
                    if x[10].lower() == weird_chain_id and x[-1] != "-"
                ]
            )
            ...
            sequence_ids[key][weird_chain_id] = {
                "similarity": max(metrics),
                "chains": chain_list,
            }
    return sequence_ids


def get_identical_sequence_weird_chains(
    data_scn, pdb_ids=None, similarity_thr=0.9, dataset="train"
):
    """
...
def get_minus_weird_chains(data_scn, pdb_ids=None, dataset="train"):
    ...
    for key in tqdm(pdb_ids):
        ...
        if len(minus_chains) > 0:
            res[key] = {
                "repeated": [
                    x for x in minus_chains if x[10].lower() in main_chain_ids
                ],
                "unique": [
                    x for x in minus_chains if x[10].lower() not in main_chain_ids
                ],
            }
    return res
...
def shorten_key_dict(key_dict, other_dict):
    ...
    for key, value in other_dict.items():
        if isinstance(value, dict):
            key_dict[key] = [
                x
                for x in key_dict[key]
                if all([x not in y for y in value.values() if isinstance(y, list)])
            ]
        else:
...
def filter_chain_pars(
    ...
    for i, res in tqdm(
        enumerate(data_scn[dataset]["res"]), total=len(data_scn[dataset]["res"])
    ):
...
def filter_weird_chains(
    ...
    dup_chain_dict = get_duplicate_weird_chains(
        data_scn, pdb_ids=list(key_dict.keys()), dataset=dataset
    )
    key_dict = shorten_key_dict(key_dict, dup_chain_dict)
    id_seq_chain_dict = get_identical_sequence_weird_chains(
        data_scn,
        pdb_ids=list(key_dict.keys()),
    ...
    to_keep = [
        x
        for x in data_scn[dataset]["ids"]
        if x not in to_remove and x not in all_to_combine
    ]
    ...
    for i in tqdm(range(len(data["ids"]))):
        for j, mask in enumerate(data["msk"][i]):
            ...
            if k < len(data["msk"][i][j]) - 1:
                ...
                data["seq"][i][j] = data["seq"][i][j][:k]
    print(
        f"Cut {cut_start} start intervals (mean length {np.array(start_lens).mean():.1f}) and {cut_end} end intervals (mean length {np.array(end_lens).mean():.1f})"
    )
    return data
